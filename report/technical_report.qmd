...
For example, during a week 7 Saints-Bears game in the 2025 regular season, the New Orleans Saints wide receiver Chris Olave caught a 57-yard catch completion to advance his team to the 20-yard line. Throughout the play, Chris Olave appears to gain yards on his defender after the ball was thrown, allowing him more space to catch the ball and secure further yards. This type of example motivates our central question: which receivers consistently create or maintain separation while the ball is in the air?
...
When controlling for other factors that influence in-air separation (defenders, teams, and situations), who are the best receivers?
...
The play-level metric we have created is an In-Air Separation Added (IASA) model:
$$
IASA = d_{catch} - d_{throw},
$$

This measures the difference in distance (in yards) between the targeted receiver and nearest defender at the time of the throw and the ball arrival (catch, pass breakup, or incompletion). Note that a positive IASA indicates the receiver gained separation from their nearest defender, while a negative IASA indicates the defender got closer. 
...
2. **Initial Separation**: There is strong regression to the mean: routes that are very open at the throw tend to see some closure by the catch, while tightly contested throws occasionally drift toward more neutral separation.
...
In all mixed-effects IASA models, we **restrict the sample to wide receivers and tight ends** and include their position as a fixed effect. To avoid over‑interpreting small‑sample noise, we fit the mixed‑effects and Bayesian models on *all* WR/TE forward‑pass targets but restrict our published leaderboards to players with at least 30 such targets. This threshold ensures that IASA‑over‑expected estimates are supported by a meaningful number of plays, while low‑volume players are still handled appropriately via partial pooling (their random effects are shrunk toward the league average and their wide credible intervals reflect the greater uncertainty).
...
if(file.exists(rankings_path)) {
  ...
  if ("targeted_name" %in% names(top_rank) && "targeted_position" %in% names(top_rank)) {
    top_rank |>
      select(nfl_id, targeted_name, targeted_position, iasa_over_expected) |>
      kable(col.names = c("ID", "Player", "Pos", "IASA Over Expected"), digits = 3)
  } else if ("displayName" %in% names(top_rank) && "targeted_position" %in% names(top_rank)) {
    top_rank |>
      select(nfl_id, displayName, targeted_position, iasa_over_expected) |>
      kable(col.names = c("ID", "Player", "Pos", "IASA Over Expected"), digits = 3)
