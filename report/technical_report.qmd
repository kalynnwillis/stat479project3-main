...
### 3.1 In‑Air Separation Added (BFD)

The play-level metric we have created is an Ball-Flight Separation added (BFD) model:
$$
...
if (sys.nframe() == 0) {
  ...
  # Apply the same football filters used for BFD modeling where possible:
...
### 3.3 Mixed‑Effects Model for BFD over Expected
...
# Goal: Predict d_catch (or BFD) using pre-throw features.
...
prepare_ml_data <- function(analysis_df) {
  ...
  # We want to predict d_catch (or BFD)
...
### 4.1 Separation Dynamics (BFD)

Across the 14,108 passing plays in our sample, receivers **typically lose separation** during ball flight. The mean BFD is **−0.82 yards**, and the distribution is centered below zero with a long left tail:
...
From a football perspective, this baseline matters: if most plays lose separation, then simply maintaining or slightly improving separation is evidence of skill. It also highlights why raw “separation at catch” can be misleading without context—some receivers routinely fight back to neutral or positive BFD from difficult starting points.

### 4.2 Drivers of Separation Change

The mixed‑effects model quantifies how air time, initial differential, and context relate to BFD. Two core drivers emerge:

1. **Air Time**: Longer throws result in more lost separation. The estimated effect of air time is negative and statistically significant, matching the intuition that deeper balls give defenders more time to recover.
2. **Initial Differential**: There is strong regression to the mean: routes that are very open at the throw tend to see some closure by the catch, while tightly contested throws occasionally drift toward more neutral separation.

![BFD vs Air Time](../figures/BFD_vs_airtime.png)
![BFD vs Initial Differential](../figures/BFD_vs_dthrow.png)

Model diagnostics show that fitted vs. observed BFD align reasonably well and residuals are roughly centered and symmetric:
...
These patterns suggest that the combination of air time, initial differential, and contextual covariates captures a meaningful portion of the variation in in‑air separation, while leaving room for receiver‑specific effects and unmeasured factors (e.g., quarterback ball placement, subtle contact downfield).
...
In all mixed-effects BFD models, we **restrict the sample to wide receivers and tight ends** and include their position as a fixed effect. To avoid over‑interpreting small‑sample noise, we fit the mixed‑effects and Bayesian models on *all* WR/TE forward‑pass targets but restrict our published leaderboards to players with at least 30 such targets. This threshold ensures that BFD‑over‑expected estimates are supported by a meaningful number of plays, while low‑volume players are still handled appropriately via partial pooling (their random effects are shrunk toward the league average and their wide credible intervals reflect the greater uncertainty).
...
#| fig-cap: "XGBoost Feature Importance for Final Separation Model"
...
if (!is.na(img_path)) {
  ...
  message("feature_importance.png not found in any expected location.")
}
```
...
### 4.5 Top Receivers (BFD Over Expected)

Finally, we leverage the mixed‑effects model to produce **receiver‑level rankings** based on BFD over expected. For each receiver with sufficient volume, we extract their estimated random effect $u_{\text{receiver}}$, which measures how much in‑air separation they add or lose relative to a context‑matched baseline.

In line with our modeling setup, these leaderboards are restricted to **wide receivers and tight ends with at least 30 forward‑pass targets** in the 2023 regular season; lower‑volume players are still included in the model fits but not in the primary rankings tables.
...
Receivers with large positive BFD‑over‑expected values consistently **beat the model**: even after accounting for air time, initial differential, and context, they end up more open at the catch than expected. The Bayesian rankings show that many of these positive effects are statistically well‑separated from zero, while players with few targets or borderline effects have much wider credible intervals, reflecting “boom‑bust” versus steady separator profiles. Conversely, receivers with negative values may rely more heavily on scheme or timing, tending to lose separation in the air relative to comparable situations. Comparing these rankings with Markov Lift highlights players who both **gain net separation** and **frequently break open** mid‑route.

![Bayesian BFD Over Expected Intervals](../src/figures/receiver_BFD_bayes_intervals.png)

Figure: Posterior mean BFD over expected and 95% credible intervals for the top receivers, distinguishing reliable separators with tight intervals from higher‑variance “boom‑bust” profiles.

## 5. Discussion

Our multi‑method analysis paints a coherent picture of in‑air receiver movement in the NFL:

1. **Baseline dynamics**: Most plays lose separation while the ball is in the air (average BFD −0.82 yards), especially on deeper, longer‑air‑time throws. This sets a challenging baseline: doing “better than zero” requires meaningful skill.
2. **Skill vs. context**: Mixed‑effects modeling shows that initial differential and air time explain a significant portion of BFD, but non‑trivial receiver‑level random effects remain, capturing genuine differences in in‑air separation skill.
...
Taken together, these results suggest that separation should be viewed as a **dynamic, shared product** of scheme, quarterback decision‑making, and receiver movement skill. BFD and Markov Lift help attribute credit within that shared process, while the Bayesian models add an explicit notion of **risk–reward** by distinguishing receivers with stable, tightly estimated advantages from those whose value comes with much wider uncertainty.

## 6. Limitations

Several limitations are important when interpreting our findings:

- **Single‑season sample**: Our analysis uses one regular season (2023). Receiver skill and team schemes evolve over time, so multi‑year stability of BFD and Markov Lift remains an open question.
- **Tracking noise and event timing**: Imperfect alignment of throw and catch frames, as well as small positional errors in tracking, introduce noise into both BFD and the frame‑level state sequences.
...
- **Attribution**: Positive BFD and Markov Lift are attributed to receivers, but in practice they also reflect quarterback play, play‑calling, and opponent quality.
...
- **Multi‑season Stability**: Track BFD and Markov Lift over multiple seasons to assess stability and year‑to‑year persistence, which is critical for scouting and contract decisions.
...
By deepening the link between tracking data and on‑field decision‑making, these extensions could make BFD and Markov Lift even more actionable for teams and analysts.
