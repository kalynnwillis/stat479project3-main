...
The league‑wide transition matrix captures how likely a receiver is to maintain or change separation from frame to frame. The diagonal elements reflect **state persistence**, while off‑diagonal elements represent gains or losses in separation. In our implementation we compute separation trajectories and transitions for **wide receivers and tight ends on forward‑pass targets**, aligning the Markov analysis with the sample used for our IASA skill modeling.
