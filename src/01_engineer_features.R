...
identify_targeted_receiver <- function(play_data) {
  ...
  if (length(tr) == 0) return(NA_integer_)  # no targeted receiver (should be rare)
  if (length(tr) > 1) {
...
calculate_separation <- function(receiver_loc, defenders_loc) {
  if (nrow(receiver_loc) == 0 || nrow(defenders_loc) == 0) return(NA_real_)
  rx <- receiver_loc$x[1]
  ry <- receiver_loc$y[1]
  dx <- defenders_loc$x
  dy <- defenders_loc$y
...
compute_play_separation <- function(gid, pid, input_df, output_df) {
  ...
  if (nrow(play_in) == 0 || nrow(play_out) == 0) return(NULL)
  
  tr_id <- identify_targeted_receiver(play_in)
  if (is.na(tr_id)) return(NULL)
...
build_play_feature_table <- function(week_data) {
  ...
  list(
    features = bind_rows(all_features),
    trajectories = bind_rows(all_trajectories)
  )
}
...
if (sys.nframe() == 0) {
  # Source the loader from the same directory
